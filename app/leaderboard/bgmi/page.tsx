...
export default function LiveLeaderboard2PageBgmi() {
    ...
    useEffect(() => {
        ...
        const fetchTeams = async () => {
            const { data, error } = await supabase
                .from("teams")
                .select("id, name, elims, alive_count, knocked_count, finishes, logo, show_on_leaderboard, outside_zone, game")
                .eq("game", "bgmi")
        ...
        const fetchSettings = async () => {
         ...
         const teamsChannel = supabase
             ...
             .on(
                 "postgres_changes",
                 { event: "*", schema: "public", table: "teams", filter: "game=eq.bgmi" },
                 (_payload) => {
                     fetchTeams();
                 }
             )
